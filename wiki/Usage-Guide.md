...
4. 核对“计划触发时间”是否正确。
5. 如系统提前拉起定时服务，任务会自动等待到计划触发时刻再执行。

## 6. 查看结果和日志

1. 检查成功记录与失败记录。
2. 打开 `日志` 页面导出 ZIP 日志或清空日志。
3. 如果自动预约失败，可在配置页底部使用手动按钮一键执行时间表中的启用时段。
4. 出现 `Unable to resolve host` 时，系统会自动每 5 秒重试，最长 5 分钟。
5. “请求频繁但最终成功”的时段不会进入失败记录区，但日志完整保留。
